...
exports.checkProjectInputs = function(inputs_key, containOutputs, containEvaluation, showError = true) {
	let checkArray = [
		...
		body(inputs_key+'.kr_title').exists().not().isEmpty().withMessage('프로젝트 한글 제목을 입력해야 합니다.')
    		.isString().trim().isLength({ min:1, max:100 }).withMessage('한글 제목은 최소 1, 최대 100글자여야 합니다.'),
		body(inputs_key+'.en_title').exists().not().isEmpty().withMessage('프로젝트 영어 제목을 입력해야 합니다.')
				.isString().trim().isLength({ min:1, max:100 }).withMessage('영어 제목은 최소 1, 최대 100글자여야 합니다.'),
				
		body(inputs_key+'.member_count').exists().not().isEmpty().withMessage('예상 팀원 수를 입력해야 합니다.')
			.isNumeric().withMessage('예상 팀원 수는 숫자여야 합니다.')
		...
		body(inputs_key+'.recruit_period.start_date').exists().not().isEmpty().withMessage('프로젝트 모집시작 기간을 입력해야 합니다.')
			.custom((value, { req }) => {
			...
			}).withMessage('프로젝트 모집 시작 날짜의 입력형식이 잘못되었습니다.'),
		body(inputs_key+'.recruit_period.end_date').exists().not().isEmpty().withMessage('프로젝트 모집 종료 날짜를 입력해야 합니다.')
			.custom((value, { req }) => {
				if (isNaN(Date.parse(value))) { return false; }
				else{
			...
			}).withMessage('프로젝트 모집 종료 날짜의 입력형식이 잘못되었습니다.')
			.custom((value, { req }) => (req.body[inputs_key].recruit_period.start_date.getTime() <= req.body[inputs_key].recruit_period.end_date.getTime())).withMessage('프로젝트 모집 종료 날짜는 프로젝트 모집 시작 날짜보다 같거나 늦어야합니다.'),
		...
		body(inputs_key+'.exec_period.start_date').exists().not().isEmpty().withMessage('수행 시작 날짜를 입력해야 합니다.')
			...
			}).withMessage('프로젝트 수행 시작 날짜의 입력형식이 잘못되었습니다.')
		...
		body(inputs_key+'.exec_period.end_date').exists().not().isEmpty().withMessage('프로젝트 수행 종료 날짜를 입력해야 합니다.')
			.custom((value, { req }) => {
				if (isNaN(Date.parse(value))) { return false; }
				else{
			...
			}).withMessage('프로젝트 수행 종료 날짜의 입력형식이 잘못되었습니다.')
		...
		.isArray({min: 0, max: 5}).withMessage('키워드 수는 최대 5개 입니다.'),
	];
