...
function getCheckMileageBase(inputs_key) {
    ...
    checkArray = [
       body(inputs_key+'.input_date').not().exists().withMessage(STR_NOT_ALLOW_ACCESS),
        body(inputs_key+'.act_date').exists().not().isEmpty().withMessage('참여일자를 입력해주세요'),
        body(inputs_key+'.act_date.from').exists().not().isEmpty().withMessage('참여시작일자를 입력해야 합니다.')
            .custom((value, { req }) => {
        ...
        body(inputs_key+'.act_date.to').exists().not().isEmpty().withMessage('참여종료일자를 입력해야 합니다.')
            .custom((value, { req }) => {
                if (isNaN(Date.parse(value))) { return false; }
                else{
        ...
        body(inputs_key+'.detail').exists().not().isEmpty().withMessage('활동상세내역을 입력해주세요!')
...
exports.checkMileageInputs = function(inputs_key, showError = true) {
    let checkArray = getCheckMileageBase(inputs_key, showError).concat([
        ...
        body(inputs_key+'.user_name').exists().not().isEmpty().withMessage('사용자 성명을 입력해주세요!')
            .isString().withMessage('사용자 성명은 문자열이여야 합니다.'),
        body(inputs_key+'.department').exists().not().isEmpty().withMessage('전공을 입력해주세요!')
        ...
        body(inputs_key+'.code').exists().not().isEmpty().withMessage('마일리지 코드값을 입력해주세요!')
...
exports.checkMileageIsAccepted = function(input_key, showError = true) {
    let checkArray = [
        body(input_key+'.is_accepted').exists().not().isEmpty().withMessage("사업단 확인여부를 체크해주세요!")
