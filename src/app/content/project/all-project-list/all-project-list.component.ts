...
@Component({
  ...
  styleUrls: ['./all-project-list.component.scss']
})
export class AllProjectListComponent implements OnInit {
  ...
  ngOnInit() {
    this.searchForm = this.formBuilder.group({
      ...
      class_name: '',
      contest_name: '',
      is_public: this.formBuilder.group({
        true: true,
        false: true 
      }),
  ...
  reloadProjects(page?) {
    ...
    this.projectService.getAllProjectCount(filter)
      .subscribe(
        (count) => {
          this.allProjectCount = count;
    ...
    this.projectService.getAllProjects(this.pageIndex * this.PROJECT_COUNT_IN_PAGE, this.PROJECT_COUNT_IN_PAGE, filter)
      .subscribe(
        (projects) => {
  ...
  createFilter() {
    ...
    filter['is_public'] = []
    if(this.true_is_public.value) { filter['is_public'].push(true); }
    if(this.false_is_public.value) { filter['is_public'].push(false); }
  ...
  gotoPage(page: number) {
    this.router.navigate(['/project/all-list', page+1]);
  ...
  get is_public() { return this.searchForm.get('is_public'); }
  get true_is_public() { return this.is_public.get('true'); }
  get false_is_public() { return this.is_public.get('false'); }
